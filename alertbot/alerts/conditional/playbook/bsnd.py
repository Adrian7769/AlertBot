...
class BSND(Base):
    def __init__(self, product_name, variables):    
        ...
        self.p_vpoc = round(variables.get(f'{self.product_name}_PRIOR_VPOC'), 2)
        self.day_open = round(variables.get(f'{self.product_name}_DAY_OPEN'), 2)
        self.p_high = round(variables.get(f'{self.product_name}_PRIOR_HIGH'), 2)
        self.p_low = round(variables.get(f'{self.product_name}_PRIOR_LOW'), 2)
        self.ib_atr = round(variables.get(f'{self.product_name}_IB_ATR'), 2)
        self.euro_ibh = round(variables.get(f'{self.product_name}_EURO_IBH'), 2)
        self.euro_ibl = round(variables.get(f'{self.product_name}_EURO_IBL'), 2)
        self.orh = round(variables.get(f'{self.product_name}_ORH'), 2)
        self.orl = round(variables.get(f'{self.product_name}_ORL'), 2)
        ...
        self.ib_high = round(variables.get(f'{product_name}_IB_HIGH'), 2)
        self.ib_low = round(variables.get(f'{product_name}_IB_LOW'), 2)
...
# ---------------------------------- Driving Input Logic ------------------------------------ #   
    def input(self):
        ...
        logger.debug(f" BSND | input | Product: {self.product_name} | LOGIC: {logic}")
