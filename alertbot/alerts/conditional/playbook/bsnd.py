...
class BSND(Base):
    def __init__(self, product_name, variables):    
        ...
        self.p_vpoc = self.safe_round(variables.get(f'{self.product_name}_PRIOR_VPOC'))
    ...
    def safe_round(self, value, digits=2):
        if value is None:
            logger.error("BSND: Missing value for rounding; defaulting to 0.")
            return 0
        try:
            return round(value, digits)
        except Exception as e:
            logger.error(f"BSND: Error rounding value {value}: {e}")
            return 0
# ---------------------------------- Specific Calculations ------------------------------------ #   
    def exp_range(self):
        
        if not self.prior_close:
            logger.error(f" BSND | exp_range | Product: {self.product_name} | Note: No Close Found")
            raise ValueError(f" BSND | exp_range | Product: {self.product_name} | Note: Need Close For Calculation!")
        ...
        if impvol is None:
            raise ValueError(f"BSND | exp_range | Product: {self.product_name} | Note: Unknown Product")
        ...
        logger.debug(f" BSND | exp_range | Product: {self.product_name} | EXP_RNG: {exp_range} | EXP_HI: {exp_hi} | EXP_LO: {exp_lo}")
        return exp_range, exp_hi, exp_lo
        
        
    def total_delta(self):
        total_delta = self.total_ovn_delta + self.total_rth_delta
        logger.debug(f" BSND | total_delta | TOTAL_DELTA: {total_delta}")
        return total_delta   
    
# ---------------------------------- Driving Input Logic ------------------------------------ #   
    def input(self):
        def log_condition(condition, description):
            logger.debug(f"BSND | input | Product: {self.product_name} | {description} --> {condition}")
            return condition
        ...
        logger.debug(f"BSND | input | Product: {self.product_name} | FINAL_LOGIC: {logic} | "
...
# ---------------------------------- Opportunity Window ------------------------------------ #   
    def time_window(self):
        ...
        if self.product_name == 'CL':
            ...
            logger.debug(f" BSND | time_window | Product: {self.product_name} | Time Window: {start_time} - {end_time}")
        elif self.product_name in ['ES', 'RTY', 'NQ']:
            start_time = self.equity_pvat_start
            end_time = self.equity_ib
            logger.debug(f" BSND | time_window | Product: {self.product_name} | Time Window: {start_time} - {end_time}")
        else:
            logger.warning(f" BSND | time_window | Product: {self.product_name} | No time window defined.")
            return False  
        if start_time <= self.current_time <= end_time:
            logger.debug(f" BSND | time_window | Product: {self.product_name} | Within Window: {self.current_time}.")
            return True
        else:
            logger.debug(f" BSND | time_window | Product: {self.product_name} | Outside Window {self.current_time}.")
            return False
# ---------------------------------- Calculate Criteria ------------------------------------ #      
    def check(self):
        ...
        if self.day_open > self.p_high:
            ...
            logger.debug(f" BSND | check | Product: {self.product_name} | DIR_LOGIC: self.day_open({self.day_open}) > self.prior_high({self.prior_high}) -> short")
        elif self.day_open < self.p_low:
            self.direction = "long"
            logger.debug(f" BSND | check | Product: {self.product_name} | DIR_LOGIC: self.day_open({self.day_open}) < self.prior_low({self.prior_low}) -> long")
        else:
            logger.debug(f" BSND | check | Product: {self.product_name} | Note: Open In Range; Not In Play, Returning.")
        ...
        if self.time_window() and self.input():
            with last_alerts_lock:
                ...
                logger.debug(f" BSND | check | Product: {self.product_name} | Current Alert: {self.direction} | Last Alert: {last_alert}")
                if self.direction != last_alert:
                    logger.info(f" BSND | check | Product: {self.product_name} | Note: Condition Met")
                    
                    # Critical Criteria Logging
                    self.c_several_dir_days = "x"
                    logger.debug(f" BSND | check | Product: {self.product_name} | Direction: {self.direction} | CRITERIA_1: Set c_several_dir_days -> [{self.c_several_dir_days}]")
                    
                    self.c_ab_vwap = "x"
                    logger.debug(f" BSND | check | Product: {self.product_name} | Direction: {self.direction} | CRITERIA_2: Set c_ab_vwap -> [{self.c_ab_vwap}]")
                    
                    self.c_posture = "x"
                    logger.debug(f" BSND | check | Product: {self.product_name} | Direction: {self.direction} | CRITERIA_3: Set c_posture -> [{self.c_posture}]")
                    
                    # Logic For c_orderflow with Logging
                    self.c_orderflow = "  "
                    if self.direction == "short" and self.delta < 0:
                        self.c_orderflow = "x"
                        logger.debug(f" BSND | check | Product: {self.product_name} | Direction: {self.direction} | CRITERIA_4: self.delta({self.delta}) < 0 for short -> [{self.c_orderflow}]")
                    elif self.direction == "long" and self.delta > 0:
                        self.c_orderflow = "x"
                        logger.debug(f" BSND | check | Product: {self.product_name} | Direction: {self.direction} | CRITERIA_4: self.delta({self.delta}) > 0 for long -> [{self.c_orderflow}]")
                    else:
                        logger.debug(f" BSND | check | Product: {self.product_name} | Direction: {self.direction} | CRITERIA_4: Orderflow criteria not met -> [{self.c_orderflow}]")
                    
                    # Logic for c_within_ibatr with Logging
                    if abs(self.cpl - self.p_vpoc) <= self.ib_atr:
                        self.c_within_ibatr = "x"
                        logger.debug(f" BSND | check | Product: {self.product_name} | Direction: {self.direction} | CRITERIA_5: |abs(self.cpl({self.cpl}) - self.prior_vpoc({self.prior_vpoc})| <= self.ib_atr({self.ib_atr}) -> [{self.c_within_ibatr}]")
                    else:
                        self.c_within_ibatr = "  "
                        logger.debug(f" BSND | check | Product: {self.product_name} | Direction: {self.direction} | CRITERIA_5: |abs(self.cpl({self.cpl}) - self.prior_vpoc({self.prior_vpoc})| > self.ib_atr({self.ib_atr}) -> [{self.c_within_ibatr}]")
                    ...
                    logger.debug(f" BSND | check | Product: {self.product_name} | Direction: {self.direction} | SCORE: {self.score}/5")
                    
                    try:
                        last_alerts[self.product_name] = self.direction
                        self.execute()
                    except Exception as e:
                        logger.error(f" BSND | check | Product: {self.product_name} | Note: Failed to send Slack alert: {e}")
                else:
                    logger.debug(f" BSND | check | Product: {self.product_name} | Note: Alert: {self.direction} Is Same")
        else:
            logger.debug(f" BSND | check | Product: {self.product_name} | Note: Condition(s) Not Met")
...
# ---------------------------------- Alert Preparation------------------------------------ #  
    def discord_message(self):
        ...
        if not settings:
            raise ValueError(f" BSND | discord_message | Note: Invalid direction '{self.direction}'")
        
        # Title Construction with Emojis
        title = f"**{self.product_name} - Playbook Alert** - **BSND** {settings['emoji_indicator']}"
    ...
    def execute(self):
        ...
        logger.info(f"BSND | execute | Product: {self.product_name} | Note: Alert Sent To Playbook Webhook")
