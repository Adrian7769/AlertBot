...
# ---------------------------------- Specific Calculations ------------------------------------ #   
    def exp_range(self):
        ...
        if not self.prior_close:
            logger.error(f" TRFD | exp_range | Product: {self.product_name} | Note: No Close Found")
            raise ValueError(f" TRFD | exp_range | Product: {self.product_name} | Note: Need Close For Calculation!")
        ...
        if impvol is None:
            raise ValueError(f" TRFD | exp_range | Product: {self.product_name} | Note: Unknown Product")
        ...
        logger.debug(f" TRFD | exp_range | Product: {self.product_name} | EXP_RNG: {exp_range} | EXP_HI: {exp_hi} | EXP_LO: {exp_lo}")
    ...
    def total_delta(self):
        ...
        logger.debug(f" TRFD | total_delta | TOTAL_DELTA: {total_delta}")
        return total_delta   
...
# ---------------------------------- Driving Input Logic ------------------------------------ #   
    def input(self):
        ...
        logger.debug(f" TRFD | input | Product: {self.product_name} | LOGIC: {logic}")
...
# ---------------------------------- Opportunity Window ------------------------------------ #   
    def time_window(self):
        ...
        if self.product_name == 'CL':
            ...
            logger.debug(f" TRFD | time_window | Product: {self.product_name} | Time Window: {start_time} - {end_time}")
        elif self.product_name in ['ES', 'RTY', 'NQ']:
            start_time = self.equity_pvat_start
            end_time = self.equity_ib
            logger.debug(f" TRFD | time_window | Product: {self.product_name} | Time Window: {start_time} - {end_time}")
        else:
            logger.warning(f" TRFD | time_window | Product: {self.product_name} | No time window defined.")
            return False  
        
        # Check if current time is within the window
        if start_time <= self.current_time <= end_time:
            logger.debug(f" TRFD | time_window | Product: {self.product_name} | Within Window: {self.current_time}.")
            return True
        else:
            logger.debug(f" TRFD | time_window | Product: {self.product_name} | Outside Window {self.current_time}.")
            return False
# ---------------------------------- Calculate Criteria ------------------------------------ #      
    def check(self):
        ...
        if self.input() and self.time_window():
            ...
            with last_alerts_lock:
                ...
                logger.debug(f" TRFD | check | Product: {self.product_name} | Current Alert: {self.direction} | Last Alert: {last_alert}")
                
                if self.direction != last_alert: 
                    logger.info(f" TRFD | check | Product: {self.product_name} | Note: Condition Met")
                    ...
                    try:
                        ...
                        logger.error(f" TRFD | check | Product: {self.product_name} | Note: Failed to send Slack alert: {e}")
                else:
                    logger.debug(f" TRFD | check | Product: {self.product_name} | Note: Alert: {self.direction} Is Same")
        else:
            logger.info(f" TRFD | check | Product: {self.product_name} | Note: Condition Not Met")
# ---------------------------------- Alert Preparation------------------------------------ #  
    def discord_message(self):
        ...
        if not settings:
            raise ValueError(f" TRFD | discord_message | Note: Invalid direction '{self.direction}'")
    ...
    def execute(self):
        ...
        logger.info(f"TRFD | execute | Product: {self.product_name} | Note: Alert Sent To Playbook Webhook")
        alert_details = {
